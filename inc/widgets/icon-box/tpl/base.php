...
$wrapper_style = '';
if ( $instance['widget_background'] == 'bg_image' && $instance['background_image'] != '' ) {
	$wrapper_style    .= ' background-image: url(\''.wp_get_attachment_url( $instance['background_image'] ).'\');';
	$class_bg_video = ' background-image';
}


$prefix = '<div class="wrapper-box-icon' . $class_bg_video . ' ' . $instance['layout_group']['text_align_sc'] . ' ' . $instance['layout_group']['box_icon_style'] . $thim_animation . '" ' . $data_color . ' style="'.$wrapper_style.'">';
$suffix = '</div>';
//wrapper-box-icon
...
if ( $instance['desc_group']['content'] != '' ) {
	if ( $instance['desc_group']['custom_font_size_des'] ) {
		$line_height = (int) $instance['desc_group']['custom_font_size_des'] + 9;
	}
