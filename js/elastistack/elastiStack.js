...
;( function( window ) {
	...
	ElastiStack.prototype._setStackStyle = function() {
		...
		if( item1 ) {
			...
			classie.add( item1, 'active' );
		}

		if( item2 ) {
			item2.style.opacity = 1;
			item2.style.zIndex = 3;
			setTransformStyle( item2, is3d ? 'translate3d('+x+'px,'+y+'px,0)' : 'translate(0,0)' );
			classie.remove( item2, 'active' );
		}

		if( item3 ) {
			item3.style.opacity = 1;
			item3.style.zIndex = 2;
			setTransformStyle( item3, is3d ? 'translate3d('+x*2+'px,'+y+'px,0)' : 'translate(0,0)' );
			classie.remove( item3, 'active' );
		}
	};
	...
	ElastiStack.prototype._moveBack = function( instance ) {
		...
		if( item3 ) {
			classie.add( item3, 'move-back' );
			classie.add( item3, 'animate' );
			setTransformStyle( item3, is3d ? 'translate3d('+x+'px,'+y+'px,0)' : 'translate(0,0)' );
		}
	};
	...
	ElastiStack.prototype._onDragMove = function( instance, event, pointer ) {
		if( this._outOfBounds( instance ) ) {
			...
			var y = this.options.y;
			if( item2 ) {
				setTransformStyle( item2, is3d ? 'translate3d( '+x+'px,' + ( (y + instance.position.y) * .6 ) + 'px, '+ ( instance.position.x * .6 ) + 'px )' : 'translate(' + ( instance.position.x * .6 ) + 'px,' + ( instance.position.y * .6 ) + 'px)' );
			}
			if( item3 ) {
				setTransformStyle( item3, is3d ? 'translate3d( '+x+'px,' + ( (y + instance.position.y) * .6 ) + 'px, '+ ( instance.position.x * .6 ) + 'px )' : 'translate(' + ( instance.position.x * .3 ) + 'px,' + ( instance.position.y * .3 ) + 'px)' );
			}
		}
	};
