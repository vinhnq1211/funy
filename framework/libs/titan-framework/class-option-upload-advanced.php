...
class TitanFrameworkOptionUploadAdvanced extends TitanFrameworkOption {
	...
	public function display() {
		...
		$value = $this->getValue();
		if ( !is_numeric( $value ) && filter_var($value, FILTER_VALIDATE_URL) !== TRUE ) {
		...
		if ( ! is_array( $value ) ) {
			if(is_numeric($value)) {
				$id 	= $value;
				$t 		= wp_get_attachment_image_src( $value );
				$value 	= $t[0];
			}elseif(filter_var($value, FILTER_VALIDATE_URL) === TRUE){
				$value = $this->getValue();
			}
			...
			foreach ($value as $k => $v) { 
				$id = $v;
				if ( ! empty( $v ) ) {
					$v = wp_get_attachment_image_src( $v );
